class BeermappingApi
  ...
  def self.places_in(city)
    city = city.downcase
    Rails.cache.fetch(city, expires_in: 60*60*24) { fetch_places_in(city) }
  end
  ...
  def self.location(id)
    return Rails.cache.fetch("locmap_#{id}", expires_in: 60*60*24) { fetch_location(id) }
  end
